...
写作要求：
- 不要使用具体的作品名称，用通用描述
- 重点介绍{category}类型资源的价值和特色
- 语言亲切自然，避免广告感

文章结构：
1. 开篇：介绍{category}资源的魅力
2. 特色分析：为什么这类资源值得关注（3-4点）
3. 适用人群：推荐给什么类型的用户
4. 使用建议：如何更好地享受这类资源
5. 资源获取：提供分享链接（如果有效）
6. 结语：鼓励理性使用，支持正版
...
重要提醒：
- 标题使用"精选{category}资源推荐"等通用表述
- 避免任何具体作品名称
- 包含免责声明
- 如果分享链接有效，格式：[获取资源链接]({downloadLink})

请按JSON格式返回：
{
  "title": "通用化的推荐标题",
  "excerpt": "50-80字的资源价值介绍",
  "content": "完整的推荐文章(markdown格式)",
  "tags": ["相关标签"],
  "imagePrompt": "抽象的{category}主题配图描述"
}`,
