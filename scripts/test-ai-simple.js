...
async function testGeminiAPI() {
  ...
  if (!apiKey || apiKey === '你的Gemini密钥') {
    console.log('❌ Gemini API Key 未正确设置');
    return false;
  }
...
async function testCohereAPI() {
  ...
  if (!apiKey || apiKey === '你的Cohere密钥') {
    console.log('❌ Cohere API Key 未正确设置');
    return false;
  }
...
async function main() {
  ...
  console.log('- GEMINI_API_KEY:', process.env.GEMINI_API_KEY ?
    (process.env.GEMINI_API_KEY === '你的Gemini密钥' ? '❌ 请替换为真实密钥' : '✅ 已设置') : '❌ 未设置');
  console.log('- COHERE_API_KEY:', process.env.COHERE_API_KEY ?
    (process.env.COHERE_API_KEY === '你的Cohere密钥' ? '❌ 请替换为真实密钥' : '✅ 已设置') : '❌ 未设置');
  ...
  if (geminiResult || cohereResult) {
    console.log('\n🎉 AI功能配置成功！可以开始自动写文章了！');
    console.log('\n📝 下一步操作:');
    console.log('1. 编辑 resources.json 添加要写文章的资源');
    console.log('2. 运行: npm run auto-publish');
  } else {
    console.log('\n⚠️  请检查API密钥是否正确配置');
  }
}
